...
class GenerateReport:
    ...
    #Retrieves curent date/time info
    @staticmethod
    def get_now():
        now = datetime.now()
        return now.strftime("%d/%m/%Y %H:%M:%S")

    #Sorts through results/dict list and gets best priced product
...
#Opens chrome and naviagates to Amazon.com with values saved in amazon_config
class AmazonAPI:
    ...
    #Runs script and outputs status/stage to terminal
    def run(self):
    ...
    #Opens browser, searchs term, removes necessary url attribtues, iterates through results list
    def get_products_links(self):
        ...
        links = []
        try:
            #results = result_list[0].find_elements_by_xpath('//*[@id="search"]/div[1]/div/div[1]/div[1]/span[3]/div[2]/div[5]/div/span/div/div/div[2]/div[2]/div/div/div[1]/h2/a')
            results = result_list[0].find_elements_by_xpath('//span/div/div/div[2]/div[2]/div/div/div[1]/h2/a')
    ...
    #Collects product asin
    def get_asins(self, links):
        return [self.get_asin(link) for link in links]

    #Collects inidividual product info
    ...
    #Converts pulled string price to float
    ...
    #Removes unecessary url attributes/tracking
